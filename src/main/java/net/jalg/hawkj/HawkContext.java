 ...
 * </ul>
 * <p>
 * HawkContext instances are created using a fluent API that uses a set of interfaces
 * (HawkContextBuilder_A, HawkContextBuilder_B, HawkContextBuilder_C, and {@link HawkContextBuilder}) to guide the
 * user through the process of supplying the necessary data at each step. The
 * process follows these general steps:
 * <ol>
 * <li>Provide request data.</li>
 * <li>Provide credentials.</li>
 * <li>Optionally provide timestamp and nonce. If they are not provided, they will
 * be generated by the builder's {@link HawkContextBuilder.build()} method when the target instance is created.</li>
 * <li>Optionally provide a payload and content type. If these are provided, the builder will generate the
 * hash value according to <a href="https://github.com/hueniverse/hawk#payload-validation">Hawk</a>
 * and add the hash value to the target instance during building.</li>
 * <li>Optionally (and <b>alternatively</b> to payload and content type) provide a payload hash value. This value will
 * be copied to the target instance during the building process.</li>
 * <li>Optionally supply application specific extension data.</li>
 * <li></li>
 * </ol>
 * Given an instance of the HawkContext class, the API is designed to enable tasks such as
 * <ul>
 * <li>Create a (Server-)Authorization header value</li>
 * <li>Validate a received HMAC</li>
 * <li>Validate a received body hash</li>
 * <li>Pass request and credentials information from a request handling chain to the
 * associated response handling chain.</li>
 * <li>Clone the instance to create a new builder. (Useful for creating response HawkContext
 * instance from a request HawkContext instance for generating a Server-Authorization response
 * header)</li>
 * </ul>
 * 
...
public class HawkContext {
	...
	private HawkContext(String method, String path, String host, int port, int ts,
			String nonce, String id, String key, Algorithm algorithm,
	 ...
	 * this HawkContext. For this the HMAC of the contained data is calculated and put
	 * into the header with the other parameters required bu the specification.
	 * 
	 * 
	...
	/**
	 * Create a new HawkContextBuilder_C object from this HawkContext that has request data,
	 * credentials, and ts and nonce already set.
	 * <p>
	 ...
	 * 
	 * @return HawkContextBuilder initialized with cloned data from this HawkContext.
	 */
	...
	public static class HawkContextBuilder implements HawkContextBuilder_A, HawkContextBuilder_B,
		...
		public HawkContext build() throws HawkException {
			...
			if (this.body != null) {
				...
				hash = HawkContextBuilder.generateHash(this.body, this.contentType);
			} else {
				hash = this.hash;
			}

			return new HawkContext(this.method, this.path, this.host, this.port, ts,
					nonce, this.id, this.key, this.algorithm, hash, this.ext);
		}
		...
		public static String generateHash(byte[] body, String contentType)
			...
			if (contentType == null || contentType.equals("")) {
				...
				ct = contentType.split(";")[0].trim(); // FIXME: limit 2?
			}
